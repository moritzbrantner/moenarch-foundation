...
input above: derived `Deserialize` is a representation contract, not proof that
...
| Reusable 3D value and transform math was duplicated in the legacy spatial owner. | Foundation now owns `math-geometry-3d`; related source remains in `rust-packages/crates/three-d/three-d-processing-core/src/{geometry,math,spatial_math,transform}.rs` for compatibility. | `math-geometry-3d` is the intended owner for domain-neutral points, vectors, quaternions, Euler order, matrices, rigid transforms, and affine transforms. A later migration may redirect legacy callers. Meshes, point clouds, bounds, camera poses, rays, planes, collision, scene SVG, and reconstruction stay in spatial/domain owners. Similarity/TRS conveniences require a separate demonstrated foundation API need. |
...
| Validated constructors and derived deserialization are inconsistent for several transfer types. | `numbers-core` range/config/tolerance values, `tensor-data`, and 2D/3D geometry derive `Deserialize`; direct public fields also bypass constructors in several of those owners. The 3D unit-quaternion case can violate a semantic type invariant. | Each present owner remains responsible for any later validating-deserializer or private-field compatibility decision. Until a behavior-authorizing slice changes it, callers must use constructors or explicit validation after untrusted deserialization; this audit does not silently alter wire behavior. |
