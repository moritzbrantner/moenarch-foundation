...
| Financial/risk-labelled functions use binary floating point. | `math-statistics`: relative/log returns, compounded return, drawdown, and tail risk use `f64`; `numbers-core` scalar totals/weighted means also use `f64`. | F6 adds exact integer/rational and decimal primitives for monetary and contractual calculations. Existing analytics remain floating-point and must not become money types implicitly. |
| No exact or decimal numeric primitive exists in foundation. | No `Decimal`, rational, currency, scale, or rounding-mode contract exists in the inspected packages. | F6 supplies domain-neutral exact/decimal primitives and explicit checked parsing/conversion/rounding semantics. Currency, ledger, tax, and payment domain rules remain above foundation. |
