...
/// an existing entry in `O(log n)` through its opaque [`Handle`]. Equal
/// priorities are ordered by insertion time and priority updates preserve that
/// tie-breaking order.
#[derive(Debug)]
...
impl<P: Ord, V> AddressablePriorityQueue<P, V> {
    ...
    pub fn insert(&mut self, priority: P, value: V) -> Handle {
    ...
    pub fn update_priority(&mut self, handle: Handle, priority: P) -> Result<(), InvalidHandle> {
    ...
    pub fn remove(&mut self, handle: Handle) -> Result<(P, V), InvalidHandle> {
        let index = self.resolve(handle)?;
        let entry = self.remove_at(index);
        Ok((entry.priority, entry.value))
    }

    fn allocate_handle(&mut self, heap_index: usize) -> Handle {
...
mod tests {
    ...
    fn equal_priorities_follow_insertion_order() {
        ...
        assert_eq!(queue.update_priority(first, 0), Err(InvalidHandle));
    }
    ...
    fn removed_handles_do_not_alias_reused_slots() {
        ...
        assert_eq!(queue.update_priority(stale, 1), Err(InvalidHandle));
        assert_eq!(queue.remove(stale), Err(InvalidHandle));
