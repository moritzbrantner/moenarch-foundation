...
///
/// Cue identifiers are optional. Cue text keeps its internal line breaks, while
/// surrounding whitespace is removed. Every cue must have a finite,
/// non-negative, non-backward time range.
...
///
/// A UTF-8 byte-order mark, the `WEBVTT` header, optional cue identifiers, and
/// cue settings after an end timestamp are accepted. Every cue must have a
/// finite, non-negative, non-backward time range.
...
fn parse_subtitles(input: &str, syntax: SubtitleSyntax) -> Result<TimedTextContract> {
    ...
    let body = strip_webvtt_header(without_bom, syntax)?;
    ...
    for (ordinal, block) in blocks.iter().enumerate() {
        let timing_index = block
            ...
            .ok_or_else(|| {
                invalid_error(format!(
                    "subtitle cue {} is missing a time range",
                    ordinal + 1
                ))
            })?;
        if timing_index > 1 {
        ...
        let (start, end) = parse_timing_line(block[timing_index], syntax, ordinal + 1)?;
        let text = block[(timing_index + 1)..].join("\n");
        let text = text.trim();
        if text.is_empty() {
            return invalid(format!("subtitle cue {} has no text", ordinal + 1));
        }

        let index = block
            .first()
            .filter(|_| timing_index == 1)
            .and_then(|identifier| identifier.trim().parse::<u64>().ok())
...
fn strip_webvtt_header(input: &str, syntax: SubtitleSyntax) -> Result<&str> {
    let Some(first_line_end) = input.find('\n') else {
        if matches!(syntax, SubtitleSyntax::WebVtt) && is_webvtt_header(input) {
            return Ok("");
        }
        return Ok(input);
    };
    let first_line = &input[..first_line_end];
    if is_webvtt_header(first_line) {
        if matches!(syntax, SubtitleSyntax::Srt) {
            return invalid("an SRT document must not contain a WEBVTT header");
        }
        Ok(&input[(first_line_end + 1)..])
    } else {
        Ok(input)
    }
}
...
fn parse_timing_line(line: &str, syntax: SubtitleSyntax, cue_number: usize) -> Result<(f64, f64)> {
    ...
    let start = parse_timestamp(start_text, cue_number)?;
    let end = parse_timestamp(end_text, cue_number)?;
    if end < start {
...
fn parse_timestamp(timestamp: &str, cue_number: usize) -> Result<f64> {
    ...
    let (hours, minutes, seconds_text) = match parts.as_slice() {
        [minutes, seconds] => (0_u64, parse_digits(minutes, cue_number)?, *seconds),
        [hours, minutes, seconds] => (
            parse_digits(hours, cue_number)?,
            parse_digits(minutes, cue_number)?,
            *seconds,
        ),
        _ => {
            return invalid(format!(
    ...
    let separator = seconds_text.find([',', '.']);
    let (seconds_part, fraction_part) = separator.map_or((seconds_text, None), |index| {
        (&seconds_text[..index], Some(&seconds_text[(index + 1)..]))
    });
    if seconds_part.is_empty()
        || !seconds_part.bytes().all(|byte| byte.is_ascii_digit())
        || fraction_part.is_some_and(|fraction| {
            fraction.is_empty() || !fraction.bytes().all(|byte| byte.is_ascii_digit())
        })
        || fraction_part.is_some_and(|fraction| fraction.contains([',', '.']))
    {
        return invalid(format!(
            "subtitle cue {cue_number} has a malformed timestamp"
        ));
    }
    let seconds = parse_digits(seconds_part, cue_number)?;
    if seconds >= 60 {
        return invalid(format!(
    ...
    let fraction = match fraction_part {
        Some(digits) => format!("0.{digits}").parse::<f64>().map_err(|_| {
            invalid_error(format!(
                "subtitle cue {cue_number} has a malformed timestamp fraction"
            ))
        })?,
        None => 0.0,
    };
    let value = (hours as f64 * 3_600.0) + (minutes as f64 * 60.0) + seconds as f64 + fraction;
    if !value.is_finite() {
        return invalid(format!(
            "subtitle cue {cue_number} timestamp must be finite"
        ));
    }
    Ok(value)
}
...
fn format_subtitles(contract: &TimedTextContract, syntax: SubtitleSyntax) -> Result<String> {
    ...
    for (index, segment) in contract.segments.iter().enumerate() {
        ...
        output.push_str(&format_timestamp(start, syntax)?);
        output.push_str(" --> ");
        output.push_str(&format_timestamp(end, syntax)?);
        output.push('\n');
...
fn format_timestamp(seconds: f64, syntax: SubtitleSyntax) -> Result<String> {
    let milliseconds = rounded_milliseconds(seconds)?;
