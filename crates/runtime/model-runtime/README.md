...
model-specific artifact metadata, and runtime access helpers. Generic artifact storage and
...
- `model.executionPlan` validates a `ModelAccessRequest` and returns a pure
  access plan with runtime execution metadata.
