...
def validate(metadata: dict, ownership: dict, root: Path = ROOT) -> list[str]:
    ...
    if len(records) != 64:
        errors.append(f"ownership must contain exactly 64 packages, found {len(records)}")
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("repository boundaries pass: 64 uniquely owned foundation packages; no path escapes or moving Git dependencies")
    return 0
