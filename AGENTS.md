...
Use `python3 scripts/coding_tooling_loop.py` for bounded repository-local remediation driven by deterministic `coding-tooling` findings. The loop reads `coding-tooling remediation plan --json`, takes one highest-priority candidate at a time, applies declared deterministic scaffolds directly, and delegates implementation candidates to a non-interactive `codex exec` when Codex is available. Set `CODING_TOOLING_LOOP_AGENT_COMMAND` or pass `--agent-command` to use another non-interactive provider command; `{prompt}` may be used as the prompt placeholder.

By default the loop considers only active new findings, processes at most five candidates, and allows at most three repair attempts per candidate. Overrides remain hard-capped at 20 candidates and five repairs so one invocation cannot become an unbounded autonomous run. Each repair fails fast through the candidate's recorded verification and then the declared `coding-tooling` fast tier. Informational `review` candidates stop for explicit review rather than authorizing an automatic edit. Once no remediation candidates remain, the loop runs `scripts/check-fast.sh` and then the `coding-tooling` full tier. `--include-baseline` is an explicit debt-remediation opt-in; do not use it casually to reopen historical debt.

The loop is a working-tree remediation layer only. It requires a clean starting worktree and always performs the repository readiness check using the same resolved `coding-tooling` command as the remediation run. It must not create or switch branches, commit, push, merge, publish, tag, baseline/suppress findings, or weaken validation. `.coding-tooling.json`, `.agent-loop.toml`, `scripts/check-agent-readiness.sh`, `scripts/check-fast.sh`, and the workspace CI workflow are protected from automatic edits unless the selected remediation candidate itself names the control file as related evidence. Failed mutation commands do not bypass these Git/control-file invariants. This loop does not replace issue-owned feature planning, final exact-head CI evidence, or release authorization.
